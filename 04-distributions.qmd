...
and many statistics courses start with coin tossing or dice rolls.
We begin with a small classroom experiment.

::: aside
Researchers found out that coin flipping does not exactly result in a 50 : 50 chance.   [$\rightarrow$ youtube video](https://youtu.be/AYnJv68T3MM)
:::
...
## Computer simulations with random numbers
...
axis(1, at=c(0, 1), label=c(0, "1/(xmax-xmin)"))
box()
```
...
## Density and quantiles of the standard normal distribution
...
type of distribution. Though this sounds quite easy, it appears
...
-- the $p$-value is greater than 0.05, so we would keep $H_0$ 
...
Recommendation: Don't use Shapiro-Wilks if you don't know the details. Use graphical checks like the Q-Q-Plot instead.
...
* $x'=\lg(x+3/8)$
* $x'=\ln(\ln(x))$ [for giant numbers]{.gray}

[$\rightarrow$ consider a GLM with family Poisson or quasipoisson instead]{.blue}
...
## Sample intervals and Confidence intervals
...
estimate the interval in which the true mean is found with 95\,\%
probability, that is the 95\,\% confidence interval:
...
The true mean ($\mu$=50) is in the interval CI = (`r  round(m + qt(c(0.025, 0.975), n-1) * se, 1)`).
