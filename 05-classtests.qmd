...
[Note: Different meaning of **significance** (H0 unlikely) and **relevance**
...
* Hubbard, R. (2004). Blurring the Distinctions Between p’s and a’s in
Psychological Research, Theory Psychology June 2004 vol. 14 no. 3
...
* If this p-value is small, below a critical value $\alpha=0.05$, we conclude it 
  is likely than an effect occured.

* The p-value is an important tool in classical statistics, but its abuse
  can lead to mis-interpreatations.
------------------------------------------------------------------------------->
...
Significance is not everything. It is often more important to focus on effect size and relevance.
...
#| fig-cap: "Visualization of a one-sample t-test. Left: original distribution of the data measured by standard deviation, right: distribution of mean values, measured by its standard error."
...
That means that both samples differ significantly ($p < 0.05$). It has
to be mentioned that **R** did not perform the "ordinary" t-test but the
Welch test (also termed heteroscedastic t-test), for which the
variances of both samples are not required to be identical.
...
The rank differences contain doubled values:
...
```{r}
  ...
  set.seed(123)
  x<-rgamma(100, 2, 1)
  plot(seq(0,1, length=100), sort(x), xlab="p", ylab=expression(sort(x[i])),
...
Let $x_{obs}$ be $`r xx`$ in our example, then $\Rightarrow$ $p= `r round(qxx,2)`$.
...
* mportant: use **absolute mesurements** (true numbers!), not percentages
...
```{r, fig=TRUE,echo=FALSE,width=6,height=4}
library(dgof)
...
|$+1 \text{resp.} -1$ | strictly positive resp. negative dependence|
...
$t=0.829 * \sqrt{1000-2}/\sqrt{1-0.829^2}=46.86, df=998$
...
[Computer statistics packages use a special algorithm (algorithm AS 89 according to Best and Roberts, 1975).]{.gray}
...
* [Example:]{.blue} Chl-a=$f(X_1, X_2, X_3, X_4, \dots)$, where $X_i$ is the
...
* Depends on:
    * the relative effect size (effect/standard deviation,
...
[$\Rightarrow$ either a large sample size or a large effect size.]{.darkred}
