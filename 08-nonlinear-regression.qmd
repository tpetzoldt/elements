...
# Some Basics of Nonlinear Regression
...
with $y$: dependent variable, $\mathbf x$: matrix of independent variables,<br>
$\mathbf p$: parameter vector, $n$: sample size.

**Use of an iterative solver**
...
The minimum of the squared residuals (SQ) is searched by [iteration]{.blue}:
...
```{r nlregexp, echo = TRUE, eval = TRUE, fig.width=6, fig.height=4}
# example data
x <- 1:10
...
* t- and p-values: can be misleading in the nonlinear case<br>
  because "non-significant" parameters can be structurally necessary
...
```{r mmselfstart, fig.width=6, fig.height=4}
```
