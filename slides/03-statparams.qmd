...
The pseudomedian ($\tilde{x}^*$) is a robust and efficient estimator for the location parameter.
It is calculated as the median of all possible means of two observations each, where the pairs include the self-pairs ($i=j$).
...
```
[Note: Strictly speaking, the population parameter is referred to as the ‘pseudomedian’ and the sample parameter as the ‘Hodges-Lehmann estimator’.]{.smallfont}
