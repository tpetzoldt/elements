...
```{r uniform-cdf, fig.width=4, fig.height=4}
```
[Cumulative distribution function]{.blue}
:::
...
Recommendation: [Use graphical checks.]{.red} Don't trust the Shapiro Wilks!
...
```{r boxcox, echo = TRUE, fig.width=5, fig.align='center'}
library(MASS)
dat <- read.csv("https://tpetzoldt.github.io/datasets/data/prk_nit.csv")
Nit90 <- dat$biovol[dat$group == "nit90"]
boxcox(Nit90 ~ 1)
...
```{r boxcox2, echo=TRUE, fig=FALSE}
bc <- boxcox(Nit90 ~ 1, plotit = FALSE)
str(bc)
bc$x[bc$y == max(bc$y)]
```

Note: these are approximate numbers so that
...
```{r boxcox3, echo=TRUE, fig.width=5, fig.align='center'}
#| label: fig-boxcox2
