...
* A hypothesis $H_a$ for the statistical relationship between the two data
  sets, is compared to an idealized null hypothesis $H_0$ that proposes no
...
* Statistical **significance** means that the null hypothesis $H_0$ is unlikely 
in a statistical sense.

* Practical **relevance** (sometimes called "practical significance") 
means that the effect size is large enough to play a role in practice. 

This means that whether an effect can be relevant or not
...
* A p-value measures the probability that a purely random effect
  would be equally or more extreme than an observed effect **if the null hypothesis is true**.
...
```{r boxplot-permutation-intro, fig.align='center'}
set.seed(123)
group1 <- rgamma(10, 2.3, 1)
group2 <- rgamma(10, 1.0, 1)
...
boxplot(value ~ group, data = dat, 
        main = "Raw Data: Two Groups", 
        xlab = "Group", ylab = "Value",
        col = c("lightblue", "lightcoral"), 
        las = 1)

# Add raw points
points(rep(1, length(group1)), group1, pch = 16, col = "blue", cex = 0.8)
points(rep(2, length(group2)), group2, pch = 16, col = "red", cex = 0.8)
```
...
1. Compute observed mean difference ($\Delta_{obs} = \bar{x}_1 - \bar{x}_2$).
2. Randomly shuffle all 20 values and reassign to two groups of 10.
3. Compute new mean difference $\Delta_{sim}$.
4. Repeat 999 times (or more) → get 999 permuted differences.
5. Compare observed difference to the distribution of permuted differences.
6. Estimate p-value as proportion of permuted differences ≥ observed.
...
n_perm <- 999
permuted_diffs <- replicate(n_perm, {
  shuffled <- sample(combined)
  mean(shuffled[1:10]) - mean(shuffled[11:20])
})

# Plot
plot(sort(permuted_diffs), type = "l", xlab = "Rank", ylab = expression(Delta),
     main = "Permutation Distribution", col = "blue", lwd = 1)

q_observed <- approx(sort(permuted_diffs), seq(0, 1000, length=n_perm), observed_diff)$y
  arrows(0, round(observed_diff, 1), q_observed, observed_diff, col="red")
  arrows(q_observed, observed_diff, q_observed, -2, col="red")
  points(q_observed, observed_diff, pch=16, col="red")
  
text(20, observed_diff, paste("Delta obs =", round(observed_diff, 1)), col="red", adj=c(0, 0))

```

Let $\Delta_{obs}$ be $`r round(observed_diff, 1)`$ in our example, then $\Rightarrow$ $p= `r 1- round(q_observed/1000,2)`$.
...
[$\Rightarrow$ we need either a large sample size or a strong effect.]{.darkred}
...
2. Expected frequencies $E_{ij} = s_i \cdot s_j / n$ [(balanced distribution = null hypothesis)]{.gray}
...
3. Test statistic $\hat{\chi}^2 = \sum_{i, j} \frac{(O_{ij} - E_{ij})^2}{E_{ij}}$
  

4. Compare with critical $\chi^2$ from table [with $(n_{row} - 1) \cdot (n_{col} - 1)$ df.]{.gray}
...
the standard $\chi^2$-test, but both are not significant.
...
*  Use of cumulative distribution function [respects order of classes]{.blue} $\rightarrow$ more powerful, than $\chi^2$-test.
...
**Philosophical problem:** We want to [keep]{.red} the $H_0$!
...
**Ratios and percentages** [values between (0, 1)]{.gray}
...
* Example: biovolumes of diatom algae cells [(species *Nitzschia acicularis*)]{.gray}.
...
More about **Nitzschia** in [Wikipedia](https://en.wikipedia.org/wiki/Nitzschia)

Data set [`prk_nit.csv`]("https://github.com/tpetzoldt/datasets/blob/main/data/prk_nit.csv") and metadata [`prk_nit_info.txt`]("https://github.com/tpetzoldt/datasets/blob/main/data/prk_nit_info.txt") available from [https://github.com/tpetzoldt/datasets](https://github.com/tpetzoldt/datasets).
:::
...
\end{cases}
$$
In some practical cases, one can simply use $y^\lambda$ instead of $\frac{y^\lambda -1}{\lambda}$.
...
* dotted vertical lines and horizontal 95%-line show the
confidence limits for possible transformations.
* Numbers are apporoximate $\rightarrow$ round it to one decimal.
* Here we can use either a **log** transformation ($\lambda=0$) or a power of
 $\approx 0.5$.
:::
...
* To test for joint distribution of all groups at once, specify explanatory variables
at the right hand side of the model formula: `biovol ~ group`
