...
* rule of thumb: the "AIC unit" is 2, so differences around 2.0 are attributed to minor differences between models
...
* Model selection identified `fert` and `light` as necessary explanatory variables,<br>
in contrast to the classical ANOVA table where only `fert` was significant.
...
* Model selection vs. p-value based testing
    ...
    * applicable for balanced and unbalanced cases
