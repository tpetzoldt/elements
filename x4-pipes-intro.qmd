...
* two samples of Maple (*Acer platanoides*) leaves, collected by two groups of students
...
::: aside
`aggregate` is very powerful, but the modern "tidyverse" approach is easier to understand.
:::
...
::: aside
The `%>%-pipeline was introduced by the user-contributed **magrittr** package [@magrittr] and 
became very popular. It is automatically loaded by the **dplyr** package [@dplyr].
:::
...
```{r echo=FALSE}
library("tidyr")
leaves |> pivot_longer(c("length", "width", "stalk")) |> head(11) |> knitr::kable()
```
:::
...
* "R for Data Science" [@datascience] 
* "ggplot Elegant Graphics for Data Analysis" [@ggplot]
